...
import 'package:flutter_ui_kit/restaurant/view/screens/welcome_screen/welcome_screen.dart';
import 'package:provider/provider.dart';

class RestOnBoardingScreen extends StatelessWidget {
  final PageController _pageController = PageController();

  @override
  Widget build(BuildContext context) {
    Provider.of<RestOnBoardingProvider>(context, listen: false).initBoardingList(context);

    return Scaffold(
      body: Consumer<RestOnBoardingProvider>(
        builder: (context, onBoardingList, child) => Column(
          children: [
            ...
            Row(
              ...
              children: [
                onBoardingList.selectedIndex != 2 ? TextButton(
                    onPressed: () {
                  ...
                  onBoardingList.selectedIndex == 2
                      ? Padding(
                          padding: EdgeInsets.only(
                              left: Dimensions.PADDING_SIZE_DEFAULT, right: Dimensions.PADDING_SIZE_DEFAULT, top: Dimensions.PADDING_SIZE_LARGE),
                          child: CustomButton(
  ...
  List<Widget> _pageIndicators(var onBoardingList, BuildContext context) {
    ...
    for (int i = 0; i < onBoardingList.length; i++) {
      _indicators.add(
        Container(
          width: i == Provider.of<RestOnBoardingProvider>(context).selectedIndex ? 16 : 7,
          height: 7,
          margin: EdgeInsets.only(right: 5),
          decoration: BoxDecoration(
            color: i == Provider.of<RestOnBoardingProvider>(context).selectedIndex ? Theme.of(context).primaryColor : ColorResources.getGrayColor(context),
            borderRadius: i == Provider.of<RestOnBoardingProvider>(context).selectedIndex ? BorderRadius.circular(50) : BorderRadius.circular(25),
          ),
        ),
      );
    }
