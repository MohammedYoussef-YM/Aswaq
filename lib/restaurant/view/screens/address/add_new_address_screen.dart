...
import 'package:flutter_ui_kit/restaurant/view/screens/address/select_location_screen.dart';
...
class AddNewAddressScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    Provider.of<RestLocationProvider>(context, listen: false).initializeAllAddressType(context: context);
    Provider.of<RestLocationProvider>(context, listen: false).updateAddressStatusMessae(message: '');
    Provider.of<RestLocationProvider>(context, listen: false).updateErrorMessage(message: '');

    if (isEnableUpdate && address != null) {
      Provider.of<RestLocationProvider>(context, listen: false)
      ...
      if (address.addressType == 'Home') {
        Provider.of<RestLocationProvider>(context, listen: false).updateAddressIndex(0);
      } else if (address.addressType == 'Workplace') {
        Provider.of<RestLocationProvider>(context, listen: false).updateAddressIndex(1);
      } else {
        Provider.of<RestLocationProvider>(context, listen: false).updateAddressIndex(2);
      }
    }
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Consumer<RestLocationProvider>(
          builder: (context, locationProvider, child) {
            if (locationProvider.address != null) {
              _locationController.text = '${locationProvider.address.featureName ?? ''} '
                  '${locationProvider.address.subAdminArea ?? ''} '
                  '${locationProvider.address.countryCode ?? ''}';
            }
                              ...
                              children: [
                                GoogleMap(
                                  ...
                                  onMapCreated: (GoogleMapController controller) {
                                    ...
                                    if (_controller != null) {
                                      Provider.of<RestLocationProvider>(context, listen: false).getCurrentLocation(mapController: _controller);
                                    }
                                  },
                                ),
                                Container(
                    ...
                    onTap: () {
                      AddressModel addressModel = AddressModel(
                        ...
                        latitude: isEnableUpdate
                            ? locationProvider.position.latitude.toString() ?? address.latitude
                      ...
                      if (isEnableUpdate) {
                        ...
                        print(address.id);
                        locationProvider.updateAddress(addressModel: addressModel, addressId: addressModel.id).then((value) {});
                      } else {
