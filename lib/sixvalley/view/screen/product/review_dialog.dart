...
import 'package:flutter_ui_kit/sixvalley/data/model/body/review_body.dart';
      ...
      child: Container(
        ...
        child: Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, mainAxisAlignment: MainAxisAlignment.center, children: [
                    ...
                    itemBuilder: (context, index) {
                      return InkWell(
                        child: Icon(
                          Icons.star,
                          size: 20,
                          color: Provider.of<SixProductDetailsProvider>(context).rating < (index+1) ? Theme.of(context).accentColor
                              : ColorResources.getYellow(context),
                        ),
                        onTap: () => Provider.of<SixProductDetailsProvider>(context, listen: false).setRating(index+1),
                      );
                    },
                  ),
                ),
            ...
            Provider.of<SixProductDetailsProvider>(context).errorText != null ? Text(Provider.of<SixProductDetailsProvider>(context).errorText,
                style: titilliumRegular.copyWith(color: ColorResources.RED)) : SizedBox.shrink(),

            Builder(
              builder: (context) => !Provider.of<SixProductDetailsProvider>(context).isLoading ? Padding(
                padding: EdgeInsets.all(Dimensions.PADDING_SIZE_LARGE),
                child: CustomButton(
                  buttonText: getTranslated('submit', context),
                  onTap: () {
                    if(Provider.of<SixProductDetailsProvider>(context, listen: false).rating == 0) {
                      Provider.of<SixProductDetailsProvider>(context, listen: false).setErrorText('Add a rating');
                    }else if(_controller.text.isEmpty) {
                      Provider.of<SixProductDetailsProvider>(context, listen: false).setErrorText('Write something');
                    }else {
                      Provider.of<SixProductDetailsProvider>(context, listen: false).setErrorText('');
                      ReviewBody reviewBody = ReviewBody(
                        productId: widget.productID,
                        rating: Provider.of<SixProductDetailsProvider>(context, listen: false).rating.toString(),
                        comment: _controller.text.isEmpty ? '' : _controller.text,
                      );
                      Provider.of<SixProductDetailsProvider>(context, listen: false).submitReview(reviewBody).then((value) {
                        if(value.isSuccess) {
                          ...
                          Provider.of<SixProductDetailsProvider>(context, listen: false).setErrorText(value.message);
                        }
                      });
                    }
                  },
