...
import 'package:flutter_ui_kit/sixvalley/view/screen/product/widget/bottom_cart_view.dart';
import 'package:flutter_ui_kit/sixvalley/view/screen/product/widget/product_image_view.dart';
import 'package:flutter_ui_kit/sixvalley/view/screen/product/widget/product_specification_view.dart';
import 'package:flutter_ui_kit/sixvalley/view/screen/product/widget/product_title_view.dart';
import 'package:flutter_ui_kit/sixvalley/view/screen/product/widget/related_product_view.dart';
import 'package:flutter_ui_kit/sixvalley/view/screen/product/widget/review_widget.dart';
import 'package:provider/provider.dart';
import 'package:flutter_ui_kit/sixvalley/view/screen/product/widget/seller_view.dart';

import 'faq_and_review_screen.dart';

class SixProductDetails extends StatelessWidget {
  final Product product;
  SixProductDetails({@required this.product});

  @override
  Widget build(BuildContext context) {
    Provider.of<SixProductDetailsProvider>(context, listen: false).removePrevReview();
    Provider.of<SixProductDetailsProvider>(context, listen: false).initProduct(product);
    Provider.of<SixWishListProvider>(context, listen: false).checkWishList(product.id.toString());
    Provider.of<SixProductProvider>(context, listen: false).removePrevRelatedProduct();
    Provider.of<SixProductProvider>(context, listen: false).initRelatedProductList();
    Provider.of<SixProductDetailsProvider>(context, listen: false).getCount(product.id.toString());
    Provider.of<SixProductDetailsProvider>(context, listen: false).getSharableLink(product.id.toString());
    NetworkInfo.checkConnectivity(context);

    return Consumer<SixProductDetailsProvider>(
      builder: (context, details, child) {
        return Scaffold(
          appBar: AppBar(
            ...
            elevation: 0,
            backgroundColor: Provider.of<SixThemeProvider>(context).darkTheme ? Colors.black : Colors.white.withOpacity(0.5),
          ),
