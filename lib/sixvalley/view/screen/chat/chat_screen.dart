...
import 'package:flutter_ui_kit/sixvalley/view/screen/chat/widget/message_bubble.dart';
...
class ChatScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if(isFirstTime) {
      Provider.of<SixChatProvider>(context, listen: false).initChatList(seller.shop.id);
    ...
    return Scaffold(
      ...
      body: Column(children: [
        ...
        // Chats
        Expanded(child: Provider.of<SixChatProvider>(context).chatList != null ? Provider.of<SixChatProvider>(context).chatList.length != 0 ? ListView.builder(
          physics: BouncingScrollPhysics(),
          padding: EdgeInsets.all(Dimensions.PADDING_SIZE_SMALL),
          itemCount: Provider.of<SixChatProvider>(context).chatList.length,
          shrinkWrap: true,
          reverse: true,
          itemBuilder: (context, index) {
            List<ChatModel> chats = Provider.of<SixChatProvider>(context).chatList.reversed.toList();
              ...
              child: Card(
                ...
                child: Padding(
                  ...
                  child: Row(children: [
                    Expanded(
                      child: TextField(
                        ...
                        onChanged: (String newText) {
                          if(newText.isNotEmpty && !Provider.of<SixChatProvider>(context, listen: false).isSendButtonActive) {
                            Provider.of<SixChatProvider>(context, listen: false).toggleSendButtonActivity();
                          }else if(newText.isEmpty && Provider.of<SixChatProvider>(context, listen: false).isSendButtonActive) {
                            Provider.of<SixChatProvider>(context, listen: false).toggleSendButtonActivity();
                          }
                        },
                      ),
                    ...
                    InkWell(
                      onTap: () {
                        if(Provider.of<SixChatProvider>(context, listen: false).isSendButtonActive){
                          Provider.of<SixChatProvider>(context, listen: false).sendMessage(_controller.text);
                          _controller.text = '';
                        }
                      },
                      child: Icon(
                        Icons.send,
                        color: Provider.of<SixChatProvider>(context).isSendButtonActive ? ColorResources.getPrimary(context) : Theme.of(context).hintColor,
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return Shimmer.fromColors(
          ...
          enabled: Provider.of<SixChatProvider>(context).chatList == null,
          child: Row(
