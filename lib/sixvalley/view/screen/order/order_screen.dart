...
import 'package:flutter_ui_kit/sixvalley/view/screen/order/widget/order_widget.dart';
...
class OrderScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if(isFirstTime) {
      Provider.of<SixOrderProvider>(context, listen: false).initOrderList();
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          isGuestMode
              ? SizedBox()
              : Provider.of<SixOrderProvider>(context).pendingList != null
                  ? Consumer<SixOrderProvider>(
              ...
              : Provider.of<SixOrderProvider>(context).pendingList != null
                  ? Consumer<SixOrderProvider>(
                      builder: (context, order, child) {
                        ...
                        if (Provider.of<SixOrderProvider>(context, listen: false).orderTypeIndex == 0) {
                          orderList = order.pendingList;
                        } else if (Provider.of<SixOrderProvider>(context, listen: false).orderTypeIndex == 1) {
                          orderList = order.deliveredList;
                        } else if (Provider.of<SixOrderProvider>(context, listen: false).orderTypeIndex == 2) {
  ...
  Widget build(BuildContext context) {
    return Expanded(
      child: TextButton(
        ...
        onPressed: () => Provider.of<SixOrderProvider>(context, listen: false).setIndex(index),
        child: Container(
          height: 40,
          alignment: Alignment.center,
          decoration: BoxDecoration(
            color: Provider.of<SixOrderProvider>(context, listen: false).orderTypeIndex == index ? ColorResources.getPrimary(context) : Theme.of(context).accentColor,
            borderRadius: BorderRadius.circular(6),
          ),
          child: Text(text + '(${orderList.length})',
              style: titilliumBold.copyWith(color: Provider.of<SixOrderProvider>(context, listen: false).orderTypeIndex == index
